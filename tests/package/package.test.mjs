...
test('紧凑 HUD 保留稳定节点并统一使用半透明表面',()=>{const html=readFileSync(path.join(root,'index.html'),'utf8');assert.match(html,/--hud-surface-alpha:\s*\.82/);for(const id of ['hud-primary','time-chip','score-chip','round-chip','rage-chip','served-val','expired-val','orders','hud-status','buff-chip','event-chip','carry-chip'])assert.equal((html.match(new RegExp(`id=["']${id}["']`,'g'))||[]).length,1,id);assert.match(html,/id="hud-status"[\s\S]*id="buff-chip"[\s\S]*id="event-chip"/);});
