...
section('经典厨房：完整做菜流程');
{
  ...
  ok('layout 尺寸 13x9', L.w === 13 && L.h === 9);
  ok('灶台动态已建', ctx.state.stations['9,5'] && ctx.state.stations['9,5'].phase === 'idle');
  ok('出生点落位', Math.floor(ctx.state.players.host.x) === 2 && Math.floor(ctx.state.players.host.z) === 2);
  ...
  const tomatoRun = () => {
    ...
    walkTo(ctx, def, 'host', 2.5, 4.5);
    walkTo(ctx, def, 'host', 2.5, 2.5);
    walkTo(ctx, def, 'host', 8.5, 2.5);
    walkTo(ctx, def, 'host', 8.5, 4.5);
    if (!walkTo(ctx, def, 'host', 9.5, 4.5, { dx: 0, dz: 1 })) throw new Error('走到灶台失败');
    act(ctx, def, 'host', 'interact'); // 下锅
  };
  tomatoRun();
  tomatoRun();
  ok('锅里 2 份番茄', ctx.state.stations['9,5'].contents.length === 2);
  tomatoRun();
  const pot = ctx.state.stations['9,5'];
  ok('3 番茄入锅开始烹饪', pot.phase === 'cooking' && pot.contents.length === 3);

  // p2：取盘子待命
  walkTo(ctx, def, 'p2', 6.5, 2.5, { dx: 0, dz: 1 });
  act(ctx, def, 'p2', 'interact');
  ok('p2 拿到盘子', ctx.state.players.p2.carrying?.k === 'plate');
  ok('干净盘子减少', ctx.state.plates.clean === 3);
  walkTo(ctx, def, 'p2', 9.5, 2.5);
  walkTo(ctx, def, 'p2', 8.5, 2.5);
  walkTo(ctx, def, 'p2', 8.5, 4.5);
  walkTo(ctx, def, 'p2', 9.5, 4.5, { dx: 0, dz: 1 });
  ...
  walkTo(ctx, def, 'p2', 8.5, 4.5);
  walkTo(ctx, def, 'p2', 8.5, 6.5);
  walkTo(ctx, def, 'p2', 6.5, 6.5);
  walkTo(ctx, def, 'p2', 6.5, 7.5, { dx: 0, dz: 1 });
  ...
  // p2 洗碗（空手）
  walkTo(ctx, def, 'p2', 6.5, 6.5, { dx: 0, dz: -1 });
  ...
  walkTo(ctx, def, 'host', 8.5, 2.5);
  walkTo(ctx, def, 'host', 8.5, 4.5);
  walkTo(ctx, def, 'host', 9.5, 4.5, { dx: 0, dz: 1 });
  act(ctx, def, 'host', 'interact');
